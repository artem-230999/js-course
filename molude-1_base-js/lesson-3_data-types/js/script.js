...
* Внутри объектов можно спокойно создавать свои объекты. Визуально можно предствать как категории филльмов, подразделяются на жанры  ит юдю 
* В реальной работе приходится работать очень часто с такими структурами. Пример файл настройки json в vscode.
const obj = {
  a: 'a',
  obj_categoty: {
    obj_categoty1: {
...
describe('Проверяем типы данных на ошибки:', function() {
    ...
    const strings = storeDescription.employees.every(elem => typeof(elem) === 'string');
